...
if (( BIN_ONLY == 0 )); then
    ...
    if [[ -f make_bin_taps.py && -f make_el_tap.py ]]; then
        ...
        die "TAP helpers not found. Expected make_bin_taps.py + make_el_tap.py, or pwsh + make_taps.ps1. BIN files were built successfully."
    fi
