...
# ----------------------------------------------------------------------
# Simple TAPs containing EasyCF_MB.bin / EasyCF_EL.bin
#
# BASIC:
...
function Make-FullElementTap {
    ...
    $EasyPath  = Join-Path $Root "EasyCF_EL.bin"
    $OutPath   = Join-Path $Root "EasyCF_EL.tap"
    ...
    if ($easy.Length -gt 0x4000) {
        throw "EasyCF_EL.bin is too large for 16K staging page: $($easy.Length) bytes"
    }
    ...
    # Route general ZXi functions to the external MB03+.
    # g_zxi_000 = #00 -> external device.
    Emit @(0x01); EmitWord 0x7C3B
    Emit @(0xAF, 0xED, 0x79)
    Emit @(0x01); EmitWord 0x7D3B
    Emit @(0xED, 0x79)

    # Switch the external MB03+ to MB02+/BSDOS mode.
    # m_zxi_000 = #01. IDE mapping stays untouched here; EasyCF EL_MB02_INIT sets m_zxi_002 = #03 before ATA access.
